...
\bigskip
\subsubsection{Sampling fossil ages}

In order to sample the age of fossil specimens, we need to specify a prior for these ages. The age ranges for our fossil samples are shown in table \ref{bearFossilTable2}. Since we have no prior knowledge on where the true age of the specimen lies in the range, we will simply use a uniform distribution as prior for each fossil. 
...
To add a prior on the age of a tip, we first need to define a taxonset containing only this tip. 

\begin{framed}
Create a new taxon set for tip \textit{Agriarctos spp.} by clicking the \fbox{\textbf{+}} and select \mi{MRCA prior} in the pop-up option box.

Label the taxon set \cl{Agriarctos\_spp}.

Move the taxa \cl{Agriarctos\_spp\_5.0} to the right-hand side column, click \mi{OK} [Figure \ref{screentaxsettip}].
\end{framed}
...
We then need to specify the prior distribution to use for that tip.
...
\descriptionhead{Example -- changing the FBD parametrization}

The FBD process can only be set up in BEAUti using the diversification rate, turnover and sampling proportion as parameters. However, it may be more convenient to use the birth, death and sampling rates as parameters, in particular for epidemiological datasets. This section will show how to adjust the XML configuration file generated using BEAUti to use this alternate parametrization.
...
Note that changing the names of the actual parameters (e.g "diversificationRateFBD.t:bearsTree" to "birthRateFBD.t:bearsTree") is not required, however keeping them risks making the file confusing, thus we in this example we will change them. The names need to be changed throughout the file, wherever they appear.
The second thing to change in this section is the upper bounds on the parameters. Both the turnover and sampling proportion are limited to $\leq 1$, but this restriction does not apply to the death rate and sampling rate and needs to be removed.
...
Note that we've changed the priors to Exponential(1) for the death rate and Exponential(0.5) for the sampling rate.
