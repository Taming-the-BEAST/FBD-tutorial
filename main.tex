...
\descriptionhead{R -- Viewing and Plotting Trees}
...
Then change all of the other rates: \mi{rateAC.s}, \mi{rateAT.s}, \mi{rateCG.s}, \mi{rateGT.s}. Fore each of these, change the parameters to:  \mi{Alpha} = \cl{2} and \mi{Beta} = 0.25. [Figure \ref{screenGTRprior}, top]
\end{framed}
...
\begin{framed}
Create a new taxon set for node 5 by clicking the \fbox{\textbf{+}} and label it \cl{5\_BrownBears}.
...
\begin{figure}[h!]
\centering
\fbox{\includegraphics[width=5.7in]{figures/geoscaled_bears.pdf}}
...
FigTree is a great tree-viewing program and it also allows you to produce publication-quality tree figures. 
...
\begin{framed}
Begin by opening \textbf{R} and load the \cl{strap} package.
...
The tree shown in Figure \ref{figgeoscale} has several fossil taxa with zero-length branches, e.g., \textit{Parictis montanus} and \textit{Ursus abstrusus}. 
...
The web-based tree viewer \href{http://tgvaughan.github.io/icytree/icytree.html}{IcyTree} (by \href{https://github.com/tgvaughan}{Tim Vaughan}) is capable of plotting such trees from BEAST2 analyses (Figure \ref{icyTree}. 

\begin{figure}[h!]
\centering
\fbox{\includegraphics[width=\textwidth]{figures/bearsDivtime_icytree.pdf}}
\caption{\small A sampled-ancestor tree exported from \href{http://tgvaughan.github.io/icytree/icytree.html}{IcyTree}. The sampled-ancestor nodes are revealed by selecting the \mi{Mark singletons} and \mi{Internal node text} set to \mi{Label}. The tree was exported to a SVG file and minor manipulations were conducted in a vector graphics editoing program (Inkscape or Adobe Illustrator).}
\label{icyTree}
\end{figure}

Although the tree in Figure \ref{figgeoscale} and \ref{icyTree} looks awesome, the topology can be misleading for this particular analysis. 
Since we did not provide character data for our 14 fossil taxa, the tree topology summarized above (e.g., Figure \ref{figgeoscale}) does not adequately illustrate the degree of uncertainty in the phylogenetic relationships of the fossil lineages.
...
%An accessory program written by Alexandra Gavryushkina called \cl{Full2ExtantConvertor.jar} is included in the download files. 
...
%When executing the \mi{FullToExtantTreeConverter} program, use a threshold value of \cl{1e-5}. This accounts for the rounding error in branch lengths so that the program knows which taxa are considered extant.

\begin{framed}
Open \cl{BEAUti} and launch the accessory apps in the file menu: \mi{File\textrightarrow Launch Apps}.
...
\caption{\small The maximum clade credibility tree of \textit{extant} bears summarized by TreeAnnotator and plotted against stratigraphy using the \cl{strap} package in \cl{R}. 
...
\href{http://creativecommons.org/licenses/by/4.0/}{\includegraphics[scale=0.8]{figures/ccby.pdf}} This tutorial was written by \href{http://phyloworks.org/}{Tracy Heath} (with helpful contributions from Alexandra Gavryushkina and Rachel Warnock) for \href{http://phyloworks.org/resources/workshops.html}{workshops} on applied phylogenetics and molecular evolution and is licensed under a \href{http://creativecommons.org/licenses/by/4.0/}{Creative Commons Attribution 4.0 International License}. 
